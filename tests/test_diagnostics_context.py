...
class TestCheckPythonVersionContext:
    ...
    async def test_uses_venv_python_version(self, mock_run_uv, mock_project_info, mock_check_venv, tmp_path):
    ...
    @patch("uv_mcp.diagnostics.check_project_venv")
    @patch("uv_mcp.diagnostics.get_project_info")
    async def test_fallback_to_system_python(self, mock_project_info, mock_check_venv, tmp_path):
    ...
    @patch("uv_mcp.diagnostics.get_project_info")
    @patch("uv_mcp.diagnostics.run_uv_command")
    async def test_version_mismatch(self, mock_run_uv, mock_project_info, mock_check_venv, tmp_path):
