...
Drop the right file in your project root and every supported client picks it up automatically.

| Client | Config file | One-liner |
|---|---|---|
| **Claude Code** | `.mcp.json` | `claude mcp add uv-mcp -- uv run uv-mcp` |
| **OpenCode** | `opencode.json` | add `"mcp"` block — [full config](/uv-mcp/guides/installation/) |
...
See the [Installation guide](/uv-mcp/guides/installation/) for the complete config for each client.
